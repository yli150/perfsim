import unittest
from perfsim.common.command import RequestCmd, ComputeCmd, DspCmd
...
class TestRuntime(unittest.TestCase):
    ...
    def test_runtime_dispatch_dsp(self):
        ...
        bBC = Barrier(env, 12, 'bc', producer=b.id, consumer=c.id)
        bCD = Barrier(env, 13, 'cd', producer=c.id, consumer=d.id)
        bAE = Barrier(env, 14, 'ae', producer=a.id, consumer=e.id)
        bDE = Barrier(env, 15, 'de', producer=d.id, consumer=e.id)

        for barrier in [bAB, bAC, bBC, bCD, bAE, bDE]:
            mgr.add(barrier)

        # assign deps on command
        a.pdeps = [bAB.id, bAC.id, bAE.id]
        b.cdeps = [bAB.id]
        b.pdeps = [bBC.id]
        c.cdeps = [bBC.id, bAC.id]
        c.pdeps = [bCD.id]
        ...
        ctx.statistic.to_chrome_trace('xdsp.json')
